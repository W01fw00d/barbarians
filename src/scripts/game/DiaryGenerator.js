...
DiaryGenerator.prototype.dead = function (killer, victim) {
  ...
  const humanAi = () => {
    ...
    return `A barbarian was defeated that day by ${
      killer.name
    }.${getPostPhrase()}`;
  };
  ...
  const aiHuman = () => {
    ...
    return `${
      victim.name
    }, one of our own, was killed today by a damn barbarian.${getPostPhrase()}`;
  };
...
DiaryGenerator.prototype.conquered = function (conqueror, town, newTownName) {
  ...
  const humanAi = () => {
    ...
    return `We captured a barbarian town previously known as ${
      town.name
    }.${getPostPhrase()}`;
  };
  ...
  const aiHuman = () => {
    ...
    return `${
      town.name
    }, our town, was captured by those damn barbarians.${getPostPhrase()}`;
  };
