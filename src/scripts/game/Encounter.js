...
Encounter.prototype.updateConqueredRomanTown = function(
  iteration,
  unit,
  conqueredUnit
) {
...
Encounter.prototype.updateConqueredBarbarianTown = function(
  iteration,
  unit,
  conqueredUnit
) {
...
Encounter.prototype.updateConqueredTown = function(
  iteration,
  unit,
  conqueredUnit,
  annotation,
  ...
  switch (unit.player) {
    case 'human':
      if (unit.movements > 0 && winner !== unit){
        html = this.iconTemplates.getHumanMob(
          id,
          unit.name,
          unit.movements,
          unit.strength
        ...
        unit.movements = 0;
        html = this.iconTemplates.getUsedHumanMob(
          id,
          unit.name,
          unit.movements,
          unit.strength
    ...
    case 'ai':
      ...
      html = this.iconTemplates.getAIMob(
        id,
        unit.name,
        unit.movements,
        unit.strength
    ...
    for (i = 0; i < iterationLength && !conquered; i++) {
      ...
      if (cellId !== undefined) {
        ...
        if (((typeOfCollindantUnit === 'E') && (unit.player === 'human'))
          ...
          units[conquered].name = this.namesManager
